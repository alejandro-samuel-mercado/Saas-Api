...
class AuthController {
  ...
  async verifyEmail(req, res, next) {
    try {
      ...
      const result = await AuthService.verifyEmail(email, code, req.tenantId);
  ...
  async resendVerificationCode(req, res, next) {
    try {
      ...
      const result = await AuthService.resendVerificationCode(email, req.tenantId);
      res.status(200).json({ success: true, ...result });
    } catch (error) {
      res.status(400).json({ success: false, message: error.message });
    }
  ...
  async resetPassword(req, res, next) {
    try {
      ...
      const result = await AuthService.resetPassword(email, code, newPassword, req.tenantId);
      res.status(200).json({ success: true, ...result });
    } catch (error) {
      res.status(400).json({ success: false, message: error.message });
    }
