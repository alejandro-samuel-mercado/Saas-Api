...
class SkuService {
  ...
  async createSku(data) {
    ...
    return await prisma.$transaction(async (tx) => {
        ...
        if (barcodeType === 'EAN13' && isAutoGenerated) {
            ...
            let seqRecord = await tx.barcodeSequence.findFirst({
                 where: { tenantId }
            });

            if (seqRecord) {
                 seqRecord = await tx.barcodeSequence.update({
                      where: { id: seqRecord.id },
                      data: { lastSequence: { increment: 1 } }
                 });
            } else {
                 seqRecord = await tx.barcodeSequence.create({
                      data: { tenantId, prefix: '779999', lastSequence: 1 }
                 });
            }
        ...
        if (finalBarcode) {
            const existing = await tx.sKU.findFirst({ where: { barcode: finalBarcode } });
            if (existing) throw new Error(`El código de barras ${finalBarcode} ya está en uso.`);
        }

        if (code) {
            const existingCode = await tx.sKU.findFirst({ where: { code } });
  ...
  async updateSku(id, data) {
    ...
    return await prisma.$transaction(async (tx) => {
       ...
       const currentSku = await tx.sKU.findFirst({ where: { id: parseInt(id) } });
