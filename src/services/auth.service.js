...
class AuthService {
  ...
  async loginWithGoogle(profile, tenantId) {
  ...
  async register(data, tenantId) {
    ...
    const existing = await prisma.user.findFirst({ where: { email: normalizedEmail, tenantId } });
    if (existing) throw new Error('Este correo electrónico ya está registrado.');

    const customerRole = await prisma.role.findFirst({ where: { name: 'CUSTOMER', tenantId } });
  ...
  async verifyEmail(email, code, tenantId) {
    const normalizedEmail = email.toLowerCase();
  ...
  async resendVerificationCode(email, tenantId) {
    const normalizedEmail = email.toLowerCase();
    const user = await prisma.user.findFirst({ where: { email: normalizedEmail, tenantId } });
  ...
  async login(email, password, tenantId) {
    ...
    if ((isEmployee || isAdmin) && !hasBranch) {
        try {
            ...
            let defaultBranch = await prisma.branch.findFirst({ where: { isHeadquarters: true, tenantId } });
            if (!defaultBranch) {
                defaultBranch = await prisma.branch.findFirst({ where: { tenantId } });
            }
