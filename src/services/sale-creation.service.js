...
class SaleCreationService {
    ...
    async createSale(userId, saleData) {
        ...
        const currency = await getCurrencyByCode(activeCurrencyCode);
        ...
        const transactionResult = await prisma.$transaction(
            async (tx) => {
                ...
                for (const item of aggregatedItems) {
                    ...
                    if (itemQty % 1 !== 0) {
                        const skuCheck = await tx.sKU.findFirst({
        ...
        `;
          ...
          SELECT * FROM "SKU" WHERE id = ${skuIdInt} AND "tenantId" = ${activeTenantId} AND "isDeleted" = false
        `;
                    const sku = skuRows[0];
             ...
             `;
                    ...
                    if (inventory.costPrice && Number(inventory.costPrice) > 0) {
                        ...
                        if (supplierSkus.length > 0) {
                            ...
                            if (sCurrency === baseCurrencyCode) {
                                ...
                                const currencyRow =
                                    await tx.$queryRaw`SELECT "exchangeRateToBase" FROM "Currency" WHERE "code" = ${sCurrency} AND "tenantId" = ${_tenantId}`;
                ...
                if (userId) {
                    ...
                    user = await tx.user.findFirst({ where: { id: userIdInt } });
                    if (!user) {
        ...
        if (sale.paymentStatus !== "PAID") {
            if (sale.paymentType === "WHATSAPP") {
                const { getStoreConfig } = require('../utils/tenant-helpers');
                const storeConfig = await getStoreConfig();
                const whatsappNumber = storeConfig?.whatsappNumber || storeConfig?.contactPhone || '';
                
                let productListText = '';
                sale.items.forEach(item => {
                    productListText += `- ${item.quantity}x ${item.name || 'Producto'} ($${item.unitPriceBase})\n`;
                });

                const customerName = customer?.name || user?.name || 'Invitado';
                const message = `*NUEVO PEDIDO - ${storeConfig?.storeName || 'Tienda'}*\n\n` +
                              `👤 *Cliente:* ${customerName}\n` +
                              `🆔 *Orden ID:* #${sale.id}\n` +
                              `📦 *Productos:*\n${productListText}\n` +
                              `🚚 *Envío:* ${deliveryType}\n` +
                              (shippingAddress ? `📍 *Dirección:* ${shippingAddress.address}, ${shippingAddress.city}\n` : '') +
                              `💳 *Método de Pago:* Acordar vía WhatsApp\n\n` +
                              `💰 *Total a pagar:* $${sale.total}\n\n` +
                              `Por favor, indíquenme los pasos para realizar el pago.`;
                              
                const encodedMessage = encodeURIComponent(message);
                const cleanNumber = whatsappNumber.replace(/[^0-9]/g, '');
                
                checkoutUrl = `https://wa.me/${cleanNumber}?text=${encodedMessage}`;
            }
            else if (["CASH", "CARD", "TRANSFER", "QR"].includes(sale.paymentType) || 
