    ...
    async updateProductPrices(productId, prices) {
        ...
        return await prisma.$transaction(async (tx) => {
            await tx.productPrice.deleteMany({
                where: { productId: parseInt(productId), tenantId }
            });
        ...
        for (const item of productsData) {
            try {
                ...
                await prisma.$transaction(async (tx) => {
                    ...
                    const existingSku = await tx.sKU.findFirst({ where: { code: skuCode } });

                    if (existingSku) {
                        // ACTUALIZAR SI YA EXISTE (UPSERT)
                        // Obtener datos actuales para fusionar
                        const existingProduct = await tx.product.findFirst({ where: { id: existingSku.productId } });
