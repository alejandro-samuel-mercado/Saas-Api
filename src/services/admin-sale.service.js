...
class AdminSaleService {
  ...
  async updateDeliveryStatus(adminId, saleId, newStatus, ip) {
      const sale = await prisma.sale.findFirst({ where: { id: parseInt(saleId) } });
      ...
      return await prisma.$transaction(async (tx) => {
          ...
          for (const item of sale.items) {
              ...
              if (branchInventoryRows.length > 0) {
                  ...
                  const invRow = await tx.branchInventory.findFirst({ where: { id: biId } });
          ...
          if (pointsEarnedInSale > 0 && sale.userId) {
              const user = await tx.user.findFirst({ where: { id: sale.userId } });
  ...
  async getDashboardStats(timeRange = 'month', branchId = null, branchIds = null) {
    ...
    const topUsers = await Promise.all(topUsersRaw.map(async (u) => {
        const user = await prisma.user.findFirst({ where: { id: u.userId }, select: { name: true, email: true } });
        return {
