const { getStoreConfig, getCurrencyByCode } = require('../utils/tenant-helpers');
const prisma = require('../config/prisma');
const PaymentGatewayFactory = require('../services/payment.factory');

class PaymentAdapter {
  
  /**
   * Crea una preferencia de pago usando la estrategia adecuada según la moneda.
   * @param {string} tenantId
   * @param {Object} sale 
   * @param {Object} user 
   * @param {string} [gatewaySlug] - Optional specific gateway to use
   * @returns {Promise<string>} URL de redirección
   */
  async createPreference(tenantId, sale, user, gatewaySlug = null) {
      try {
          // 1. Resolver Strategy
          let strategy;
          if (gatewaySlug) {
              strategy = await PaymentGatewayFactory.getGatewayBySlug(tenantId, gatewaySlug);
          } else {
              const config = await getStoreConfig();
              const currencyCode = sale.currencyCode || (config?.baseCurrency || 'USD');
              strategy = await PaymentGatewayFactory.getGateway(tenantId, currencyCode);
          }
          
          // 2. Delegar creación
          return await strategy.createPreference(sale, user);
      } catch (error) {
          console.error('[PaymentAdapter] Create Preference Error:', error);
          throw error;
      }
  }

  /**
   * Procesa un pago directo (síncrono) sin redirección (Ej. Card Tokenization)
   */
  async processPayment(tenantId, sale, user, gatewaySlug, paymentData) {
      try {
          const strategy = await PaymentGatewayFactory.getGatewayBySlug(tenantId, gatewaySlug);
          if (typeof strategy.processPayment !== 'function') {
              throw new Error(`Strategy for ${gatewaySlug} does not support synchronous processPayment`);
          }
          return await strategy.processPayment(sale, user, paymentData);
      } catch (error) {
          console.error('[PaymentAdapter] Process Payment Error:', error);
          throw error;
      }
  }

  /**
   * Reembolsa un pago.
   * @param {string} tenantId
   * @param {string} paymentId 
   * @param {number} [amount] 
   * @param {string} [currencyCode] - Necesario para saber qué gateway usar si no tenemos el objeto sale
   */
  async refundPayment(tenantId, paymentId, amount, currencyCode) {
      if (!currencyCode) {
          const config = await getStoreConfig();
          currencyCode = config?.baseCurrency || 'USD';
      }
      try {
          const strategy = await PaymentGatewayFactory.getGateway(tenantId, currencyCode);
          return await strategy.refundPayment(paymentId, amount);
      } catch (error) {
          console.error(`[PaymentAdapter] Refund Failed for ${paymentId}:`, error);
          throw error;
      }
  }
}

module.exports = new PaymentAdapter();
