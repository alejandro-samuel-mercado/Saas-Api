...
// 1. CORS: Permite peticiones de todos los orígenes
// Debe ir antes de Helmet para que los headers de CORS no sean sobrescritos o bloqueados
app.use(cors({
  origin: (origin, callback) => {
    // Permitir cualquier origen de forma dinámica para soportar credentials: true
    callback(null, true);
  },
  credentials: true,
  allowedHeaders: ['Content-Type', 'Authorization', 'X-Requested-With', 'x-branch-id', 'x-currency', 'x-idempotency-key', 'x-silence-toast', 'x-test-country', 'x-client-country', 'x-tenant-id']
}));
...
// ========== RUTAS SAAS (SIN tenant middleware) ==========
const saasRoutes = require('./routes/saas.routes');
app.use('/api/saas', saasRoutes);

// Ruta de plan info para panels que ya tienen tenant (vía x-tenant-id)
...
app.get('/', (req, res) => {
  res.status(200).json({ 
    status: 'success', 
    message: 'API E-commerce SaaS Multi-Tenant',
    timestamp: new Date()
  });
});
...
app.use('/api/cash-registers', checkMaintenanceMode, cashRegisterRoutes);
